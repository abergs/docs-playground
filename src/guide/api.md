...
### Request
`POST` requests made to the `/signin` endpoint unpack a [verification token](concepts.html#tokens), which must be generated by calling a `.signin()` method on your frontend ([learn more](js-client.html#signin)) and included here in the response body, for example:
